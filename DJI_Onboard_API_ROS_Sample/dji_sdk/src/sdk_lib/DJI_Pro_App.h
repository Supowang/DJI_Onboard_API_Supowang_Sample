...
#define API_CTRL_GIMBAL_ANGLE   0x1B
...
typedef struct 
{
    uint8_t cur_ctrl_dev_in_navi_mode : 3;
    uint8_t serial_req_status         : 1;
    uint8_t reserved                  : 4;
}ctrl_device_t;
...
typedef struct 
{
  ...
  uint8_t duration;     //camera would turn the set angle in x * 0.1 s;
}gimbal_custom_control_angle_simple_t ;

/* gimbal angle data struct*/
typedef struct
{
	int16_t yaw_angle;
	int16_t roll_angle;
...
typedef struct
{
    int16_t yaw_angular_rate;
    int16_t roll_angular_rate;
    int16_t pitch_angular_rate;
    struct
     {
        uint8_t reserve            :7;
        uint8_t ctrl_switch        :1;
     }ctrl_byte;
}gimbal_custom_control_speed_t;
